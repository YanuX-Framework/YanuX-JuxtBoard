    ...
    try {
        ...
        if (idToken) {
            ...
            if (!authentication.accessToken && code && codeVerifier) {
                ...
                dispatch(exchangedAuthorizationCode(code, resposeJson));
                dispatch(readyToConnect(resposeJson.access_token));
            } else if (authentication.accessToken) { dispatch(readyToConnect(authentication.accessToken)); }
